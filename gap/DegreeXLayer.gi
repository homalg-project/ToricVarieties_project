...
##         formated for use in MAGMA
##
######################################################################################################

# compute degree X layer of projective graded S-module morphism and write the corresponding matrix to a file
InstallMethod( WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForMAGMA,
               " a toric variety, a projective graded module morphism, a list specifying a degree ",
               [ IsToricVariety, IsCAPCategoryOfProjectiveGradedLeftOrRightModulesMorphism, IsList, IsString ],
  function( variety, projective_module_morphism, degree, filename )
    local left, gens_source, gens_range, dim_range, mapping_matrix, counter, rationals, i, comparer, j, non_zero_rows,
         poly, poly_split, poly_split2, k, pos, coeff, l, name_of_indeterminates, vector_space_morphism, split_pos,
         path, file, stream;
    ...
    if Length( gens_source ) = 0 then
      ...
      # inform that we are starting the computation
      Print( "Starting the matrix computation now... \n" );
      ...
      file := Filename( Directory( path ), filename );
      stream := OutputTextFile( file, false ); # true means that we append to this file

      # write syntax...
      WriteLine( stream, Concatenation( filename, " := SparseMatrix( " ) );

      # write number of rows
      WriteLine( stream, Concatenation( String( dim_range ), "," ) );
      ...
      # and inform that we are done with writing the matrix
      Print( Concatenation( "Have written the matrix to file ", filename, "\n" ) );

      # and signal end of computation
      return true;
      ...
      # write number of columns
      WriteLine( stream, Concatenation( String( Length( gens_source ) ), ");" ) );
      ...
      Print( Concatenation( "NrColumns: ", String( Length( gens_source ) ), "\n" ) );
      Print( Concatenation( "Have to go until i = ", String( Length( gens_source ) ), "\n" ) );
      ...
      # open list for all non-trivial entries
      WriteLine( stream, "[" );
      ...
      # initialise the field of rationals in MAGMA to be used
      ...
      for i in [ 1 .. Length( gens_source ) ] do
        ...
        for j in [ 1 .. Length( non_zero_rows ) ] do
          ...
          for k in [ 1 .. Length( poly_split2 ) ] do
            ...
            pos := gens_range[ non_zero_rows[ j ] ].( poly_split2[ k ][ 2 ] );
            if ( i = Length( gens_source ) ) and ( j = Length( non_zero_rows ) ) and ( k = Length( poly_split2 ) ) then
              WriteLine( stream,
                       Concatenation( "<", String( pos ), ",", String( i ), ",", String( poly_split2[ k ][ 1 ] ), ">" ) );
            else
              WriteLine( stream,
                       Concatenation( "<", String( pos ), ",", String( i ), ",", String( poly_split2[ k ][ 1 ] ), ">," ) );
            fi;
      ...
      # print that we are done with this step
      Print( Concatenation( "Have written the matrix to file ", filename, "\n" ) );

      # and signal a successful finish of the computation
      return true;
...
InstallMethod( WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForMAGMA,
               " a toric variety, a projective graded module morphism, a homalg_module_element specifying a degree ",
               [ IsToricVariety, IsCAPCategoryOfProjectiveGradedLeftOrRightModulesMorphism, IsHomalgModuleElement, IsString ],
  function( variety, projective_module_morphism, degree, filename )

    return WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForMAGMA(
                   variety, projective_module_morphism, UnderlyingListOfRingElements( degree ), filename );
...
# compute degree X layer of projective graded S-module morphism and write the corresponding matrix to a file
InstallMethod( WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForGAP,
               " a list of information and a string",
               [ IsList, IsString, IsBool ],
  function( infos, filename, display_messages )
    local variety, projective_module_morphism, degree, gens_source, gens_range,
         left, dim_range, matrix, mapping_matrix, counter, i, comparer, j, non_zero_rows,
         poly, poly_split, poly_split2, k, pos, coeff, l, name_of_indeterminates, vector_space_morphism, split_pos,
         path, file, stream;

    # extract the data
    variety := infos[ 1 ];
    projective_module_morphism := infos[ 2 ];
    degree := infos[ 3 ];
    gens_source := infos[ 4 ];
    gens_range := infos[ 5 ];
    ...
    # this method is called only if source and range are both non-trivial, so...
    ...
    WriteLine( stream, "rationals := HomalgFieldOfRationals();" );
    WriteLine( stream, Concatenation( filename, ":= HomalgInitialMatrix( ", String( dim_range ), ",", 
                                                  String( Length( gens_source ) ), ", rationals );" ) );
...
InstallMethod( WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForGAP,
  ...
  function( sourcefile, degree, targetfile, display_messages )
    local path, file, rationals, mor, variety, gens_source, gens_range;
    ...
    Read( file );
    mor := ValueGlobal( "mor" );
    variety := ValueGlobal( "variety" );
    gens_source := ValueGlobal( "gens_source" );
    gens_range := ValueGlobal( "gens_range" );

    # and hand it over
    return WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForGAP(
              [ variety, mor, degree, gens_source, gens_range ], targetfile, display_messages );

end );

InstallMethod( WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForGAP,
               " a string, a homalgModuleElement, a string, a ring, a bool",
               [ IsString, IsHomalgModuleElement, IsString, IsBool ],
  function( sourcefile, degree, targetfile, display_messages )

    return WriteDegreeXLayerOfProjectiveGradedLeftOrRightModuleMorphismToFileForGAP(
               sourcefile, UnderlyingListOfRingElements( degree ), targetfile, display_messages );
