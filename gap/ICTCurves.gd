################################################################################################
##
##  ICTCurves.gd         ToricVarieties package
##
...
#! @Description
#! The arguments are a smooth and complete toric variety $X_\Sigma$ and two non-negative and distinct integers $i,j$. We then consider
#! the i-th and j-th maximal cones $\sigma_i$ and $\sigma_j$. ! If $\tau := \sigma_i \cap \sigma_j$ satisfies 
#! $\text{dim} \left( \tau \right) = \text{dim} \left( \sigma_1 \right) - 1$, then $\tau$ corresponds to an ICT-curve. 
#! We then construct this very ICT-curve.
...
#! @Description
#! The argument is an ICT curve $C$. The output is a list with the intersection numbers of a canonical base of the class group. This basis
#! is to take $\left( e_1, \dots, e_k \right)$ with $e_i = \left( 0, \dots, 0, 1, 0, \dots, 0 \right) \in \text{Cl} \left( X_\Sigma \right)$.
#! @Returns a list of integers
#! @Arguments C
...
#! @Description
#! For a smooth and complete toric variety $X_\Sigma$, this method computes a list of all ICT-curves in $X_\Sigma$. Note that those
#! curves can be numerically equivalent.
...
#! @Description
#! Given an ICT-curve $C$ and a divisor $D$ in a smooth and complete toric variety $X_\Sigma$, this method computes their intersection
#! product.
#! @Returns an integer
#! @Arguments C,D
...
#! @Description
#! For a smooth and complete toric variety $X_\Sigma$, this method computes a list of all ICT-curves which are not numerically equivalent.
#! We use this list of ICT-curves as a basis of proper 1-cycles on $X_\Sigma$ in the constructor below, when computing the intersection form
#! and the Nef-cone.
#! @Returns a list of ICT-curves.
#! @Arguments X_Sigma
...
#! @Description
#! The arguments are a smooth and complete toric variety $X_\Sigma$ and a list of integers. We then use the integers in this list
#! as 'coordinates' of the proper 1-cycle with respect to the list of proper 1-cycles produced by the previous method.
#! We then return the corresponding proper 1-cycle.
...
#! @Description
#! The argument is a proper 1-cycle. We then return the underlying group element (with respect to the generators computed from the method
#! \emph{GeneratorsOfProper1Cycles}).
#! @Returns a list
#! @Arguments C
...
#! @Description
#! Given a proper 1-cycle $C$ and a divisor $D$ in a smooth and complete toric variety $X_\Sigma$, this method computes their intersection
#! product.
#! @Returns an integer
#! @Arguments C,D
DeclareOperation( "IntersectionProduct",
