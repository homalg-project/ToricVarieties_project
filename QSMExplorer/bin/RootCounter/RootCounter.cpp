...
void Distributer( WeightedDiagram dia, std::vector<int> ws )
{
...
void Myprint_vector(std::vector<int> const &vs, std::string const &message)
{
    std::cout << message;
...
// helper to print vector (of vectors) of integers
void Myprint_vector_without_break(std::vector<int> const &vs, std::string const &msg1, std::string const &msg2 )
{
    std::cout << msg1;
    for ( int i = 0; i < vs.size()-1; i++ ){
        std::cout << vs[ i ] << ",";
...
void Myprint_vector_of_vector(std::vector<std::vector<int>> const &vs, std::string const &message)
{
    std::cout << message;
    for ( int i = 0; i < vs.size(); i++ ){
            Myprint_vector_without_break( vs[ i ], "(", ") " );
    }
    std::cout << "\n";
}

int countMinimalRoots( WeightedDiagram dia, int NUM_THREADS )
{
...
int countMinimalRoots( WeightedDiagram dia )
{
    return countMinimalRoots( dia, dia.get_root() - 1 );
}
