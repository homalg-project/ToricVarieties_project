...
#include "Auxilliary/hash_table.cpp"

#include "Auxilliary/combinatorics_with_hash.cpp"
...
int main(int argc, char* argv[]) {
    ...
    // ######################################
    // ##### (1) Read input from CSV
    // ######################################
    


    customHash hashTable;

    std::chrono::steady_clock::time_point before = std::chrono::steady_clock::now();

    std::ifstream graph_file("graphs.csv");

    int i = 0;

    if (!graph_file.is_open()) throw std::runtime_error("No file found?");

    std::string input_string;
    while (std::getline(graph_file, input_string)) {
...
/*        if (i%5==0){
            ...
            auto interT = std::chrono::duration_cast<std::chrono::microseconds>(inter - before);
            std::cout << "\nCompleted " << i << " Computations at time: " << interT.count();
        }*/
        i++;
        //delete hashTable;
    }
    ...
    std::cout << "\nCompleted full computation in " << duration.count() << " microseconds";
